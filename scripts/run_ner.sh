...
train_file=${NER_DATA_DIR}/${short}.train.in.conllu
eval_file=${NER_DATA_DIR}/${short}.dev.in.conllu
output_file=${NER_DATA_DIR}/${short}.dev.${outputprefix}pred.conllu
gold_file=${NER_DATA_DIR}/${short}.dev.gold.conllu
...
python -m stanfordnlp.models.ner_tagger --wordvec_dir $WORDVEC_DIR --train_file $train_file --eval_file $eval_file \
    --output_file $output_file --batch_size $batch_size --gold_file $gold_file --lang $lang --shorthand $short \
    --mode train $args
python -m stanfordnlp.models.ner_tagger --wordvec_dir $WORDVEC_DIR --eval_file $eval_file \
...
echo $results $args >> ${NER_DATA_DIR}/${short}.results
