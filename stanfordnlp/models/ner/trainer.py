...
from stanfordnlp.models.ner.model import Tagger
...
class Trainer(BaseTrainer):
    ...
    def predict(self, batch, unsort=True):
        ...
        xpos_seqs = [self.vocab['xpos'].unmap(sent) for sent in preds[0].tolist()]

        pred_tokens = [[xpos_seqs[i][j] for j in range(sentlens[i])] for i in range(batch_size)]
        if unsort:
